...
/* -----------------------------------
   CRITICAL FIX 1: EXPORTED TOTALS FOR MAIN ESTIMATOR
   (This allows estimator-advanced.js to add the cost)
----------------------------------- */
window.hammerAddonTotals = { // <--- ADDED
  low: 0, // <--- ADDED
  high: 0 // <--- ADDED
}; // <--- ADDED
...
function applySmartAddonBreakdown() {
  ...
  window.hammerAddonTotals.low = addonLow; // <--- ADDED
  window.hammerAddonTotals.high = addonHigh; // <--- ADDED
  // ----------------------------------------------------------------------
  ...
  // CRITICAL FIX 3: Change insertion point to the parent container's end for PDF/Email compatibility.
  base.el.parentNode.insertAdjacentHTML("beforeend", breakdownHtml); // <--- MODIFIED LINE (was base.el.insertAdjacentHTML("afterend", ...)
}

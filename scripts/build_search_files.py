...
class Head(HTMLParser):
    def __init__(self, text):
        ...
        self.meta = {}; self.title = ''; self.in_title = False; self.images = []
        self.feed(text)
...
def build():
    ...
    for path in sorted(ROOT.glob('*.html')):
        ...
        report.append({'file':path.name,'title':title,'description':description,'indexable':indexed,'sitemap':bool(include),'schemaTypes':[g['@type'] for g in graph] if seo.get('structuredDataEnabled',True) else [],'warnings':(['Missing description'] if not description else [])})
    ...
    (ROOT/'admin-tools/search-readiness.json').write_text(json.dumps({'pages':report,'note':'Local validation only. Indexing, rich results and voice-assistant placement are unverified.'},indent=2)+'\n')
